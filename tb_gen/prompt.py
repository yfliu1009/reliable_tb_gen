...
# However, if you think this problem can be fixed, output NO enclosed by three asterisks and provide a corrected problem along with its solution.
...
TB_FOR_QUESTION_GUIDE = """Does the following Verilog coding problem provide enough information to generate a code that can pass the testbench?
you must consider that when generating the code, it can't get the testbench's information.
If so, output YES enclosed by three asterisks.
Otherwise, if you consider this problem unsolvable, output DROP enclosed by three asterisks.
However, if you think this problem can be fixed, output NO enclosed by three asterisks and provide a corrected problem along with its solution.
Enclose the corrected problem with [BEGIN PROB] and [END PROB].
...
# Based on the provided information, fix either the solution or the testbench to reflect the true intention of the Verilog coding problem. Indicate your decision by outputting SOLUTION or TESTBENCH enclosed by three asterisks.
...
FIX_GUIDE = """Act as a senior Verilog verification engineer.
Below is a Verilog coding problem along with an attempted solution and a draft testbench.
Reported errors when running the solution against the testbench are also shown below.
...
Below is a Verilog problem spec. Based on this spec, your task is to generate testcases that cover a broad range of scenarios.
The code coverage and the functional coverage should be as high as possible. Most importantly, the output format should be as follow:
[BEGIN]\n Testcase1: input: <input_signal_values>, output: <expected_output_signal>.\n Testcase2: ... \n[DONE]. 
The inputs and outputs will be used in testbench after. Therefore, provide the exact values instead of text descriptions.
Only provide me the testcases and nothing else.
"""

TB_TESTCASE_GUIDE = """Act as a senior Verilog verification engineer.
...
FIX_GUIDE_TESTCASE = """Act as a senior Verilog verification engineer.
Below is a Verilog coding problem along with an attempted solution and a draft testbench.
Reported errors when running the solution against the testbench are also shown below.
You must refer to this information and AST tool information highly since the testbench may have some mistakes. 
Based on the provided information, fix the testbench to reflect the true intention of the Verilog coding problem without modifying the testcases. Indicate your decision by outputting TESTBENCH enclosed by three asterisks.
Enclose your fixed code with [BEGIN] and [DONE] and do not output anything else.
example: [BEGIN]\n Testbench \n [DONE]
If you decide to fix the testbench, the testbench must print the number of passed test cases and the number of total test cases enclosed by triple backticks to the terminal.
Format: ```Number of passed test cases: 8``` and ```Number of total test cases: 10```.
